...
pub fn encode_gif(
    ...
    let fps = if options.gif_fps > 0 { options.gif_fps } else { 15 };
    let width = if options.gif_width > 0 { options.gif_width as i32 } else { -1 };
    let colors = if options.gif_colors > 0 { options.gif_colors } else { 256 };

    // 1. Generate palette
    let temp_dir = std::env::temp_dir();
    let palette_path = temp_dir.join(format!("palette_{}.png", uuid::Uuid::new_v4()));

    let palette_filter = format!("fps={},scale={}:-1:flags=lanczos,palettegen=max_colors={}", fps, width, colors);
    ...
    if !status.success() {
        return Err(FfmpegError::EncodeError("Failed to generate GIF palette".into()));
    }

    // 2. Use palette to generate GIF
    let use_filter = format!("fps={},scale={}:-1:flags=lanczos [x]; [x][1:v] paletteuse=dither=bayer", fps, width);
...
pub fn split_by_scenes(
    ...
    if !output.status.success() {
        ...
        return Err(FfmpegError::SceneDetectError(format!("FFmpeg detection pass failed: {}", err_msg)));
    }
    ...
    for (i, &end_time) in timestamps.iter().chain(std::iter::once(&999999.0)).enumerate() {
        if end_time - start_time < 0.1 && i > 0 { continue; } // skip tiny segments
        ...
        let status = cmd.status().map_err(|e| FfmpegError::EncodeError(e.to_string()))?;
        if status.success() {
            result_files.push(output_file);
        }

        if end_time > 999998.0 { break; }
        start_time = end_time;
    }
