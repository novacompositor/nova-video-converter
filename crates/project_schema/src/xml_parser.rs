...
pub fn parse_fcpxml_sequence(xml_content: &str) -> Result<Sequence, XmlParseError> {
    ...
    if let Some(spine) = sequence_node
        .descendants()
        .find(|n| n.has_tag_name("spine"))
    {
        ...
        for clip_node in spine.children().filter(|n| n.is_element()) {
            ...
            let clip =
                Clip::new_asset(name, asset_id, AssetKind::Video, start, source_in, duration);
...
fn parse_apple_time(time_str: &str) -> Result<RationalTime, XmlParseError> {
    ...
    if num_str.contains('/') {
        ...
        if parts.len() == 2 {
            let num: i64 = parts[0]
                .parse()
                .map_err(|_| XmlParseError::InvalidTime(time_str.to_string()))?;
            let den: u32 = parts[1]
                .parse()
                .map_err(|_| XmlParseError::InvalidTime(time_str.to_string()))?;
